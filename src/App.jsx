...
const SegmentPL=({incList,expList,db=null,gotoOP=null,gotoIP=null,mtdIncList=null,mtdExpList=null,mtdLabel='',mtdTitle='Month to date net profit',monthlyOf=null})=>{
    ...
    const calcSeg=(seg,iL,eL)=>{
      ...
      // Patients for each doctor from this period's entries (for context + navigation)
      Object.keys(commByDoc).forEach(dn=>{sInc.filter(e=>e.ref_doctor===dn&&getComm(e)>0).forEach(e=>{const pn=(e.patient_name||'—').trim()||'—';if(!commByDoc[dn].pats[pn])commByDoc[dn].pats[pn]={a:0,pid:null,isIP:false};commByDoc[dn].pats[pn].a+=getComm(e);if(['ip','ip_r','ip_l','ip_p'].includes(e.type)){commByDoc[dn].pats[pn].isIP=true;if(e.patient_id)commByDoc[dn].pats[pn].pid=e.patient_id}})})
