...
const DeductCommForm=({docName,balance,db,onSave,onCancel})=>{
  ...
  const dueAfter=Math.max(0,Math.round(balance)-give-ded)
  const go=async()=>{
  ...
  return(
    <div style={{background:'#fffbeb',borderRadius:10,padding:'12px 14px',border:'1px solid #fde68a',marginTop:10}}>
      ...
      <div style={{background:'#f0fdf4',border:'1px solid #bbf7d0',borderRadius:8,padding:'8px 10px',marginBottom:10,fontSize:11.5,lineHeight:1.6}}>
        ...
        {!keepDue&&<div style={{color:'#92400e',fontWeight:700}}>Auto-deducted (kept): {fmt(ded)} <span style={{fontWeight:600,color:'#a16207'}}>(🏥 {fmt(d1)} + 🧪 {fmt(d2)})</span></div>}
        <div style={{color:'#475569'}}>Doctor due after: <b style={{color:dueAfter>0?'#c2410c':'#15803d'}}>{fmt(dueAfter)}</b></div>
      </div>
      <div style={{display:'flex',gap:8}}>
        ...
        <button onClick={go} disabled={busy} style={{flex:2,padding:'9px',background:'#d97706',color:'#fff',border:'none',borderRadius:8,fontSize:12,fontWeight:800,cursor:busy?'not-allowed':'pointer'}}>{busy?'Saving...':keepDue?'Record payment':'Settle (pay + deduct)'}</button>
      </div>
    </div>)
}
    ...
    return(
      <div>
        ...
        {isPharm&&(()=>{
          ...
          return(<div style={{display:'flex',justifyContent:'space-between',alignItems:'center',background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:8,padding:'7px 12px',marginBottom:8,fontSize:11.5,fontWeight:700,flexWrap:'wrap',gap:4}}>
            <span style={{color:'#334155'}}>📋 Bills {fmt(bills)}</span>
            <span style={{color:'#94a3b8'}}>−</span>
            <span style={{color:'#16a34a'}}>✓ Collected {fmt(coll)}</span>
            <span style={{color:'#94a3b8'}}>=</span>
        ...
        {isPharm&&pharmView==='settled'&&(()=>{
          ...
          if(settledEnts.length===0)return null
          return(<div style={{background:'#fff7ed',border:'1px solid #fdba74',borderRadius:10,padding:'10px 12px',marginTop:8}}>
            <div style={{fontSize:11,fontWeight:800,color:'#c2410c',textTransform:'uppercase',letterSpacing:'.4px',marginBottom:8}}>🧾 Settlement details — when paid & where adjusted</div>
            {settledEnts.slice().sort((a,b)=>(b.date||'').localeCompare(a.date||'')).map(e=>{
              const note=e.notes||''
              const origM=note.match(/originally from ([^)·]+)/)
              return(<div key={e.id} style={{padding:'6px 0',borderBottom:'1px solid #fed7aa',fontSize:12}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
            ...
            <div style={{display:'flex',justifyContent:'space-between',paddingTop:8,fontSize:12,fontWeight:800,color:'#7c2d12'}}>
              <span>Total settled via credit</span><span style={{color:'#15803d'}}>{fmt(settledEnts.reduce((a,e)=>a+e.amount,0))}</span>
            </div>
          </div>)
        })()}</div>)})}
        {p.insurance_type&&(<>
  ...
  return(<>
    ...
    {subTab==='due'&&(()=>{
      ...
      return(<>
        ...
        {dueRows.map(r=>{return(<Card key={r.name} style={{border:'1px solid #fed7aa'}}>
...
const SegmentPL=({incList,expList,db=null,gotoOP=null,gotoIP=null,mtdIncList=null,mtdExpList=null,mtdLabel='',mtdTitle='Month to date net profit',monthlyOf=null})=>{
    ...
    const calcSeg=(seg,iL,eL)=>{
      ...
      const incByType={}
      collInc.filter(e=>!IP_T.includes(e.type)).forEach(e=>{if(!incByType[e.type])incByType[e.type]={amt:0,pats:{}};incByType[e.type].amt+=e.amount||0;const pn=(e.patient_name||'—').trim()||'—';if(!incByType[e.type].pats[pn])incByType[e.type].pats[pn]={a:0,pid:null,isIP:false,comm:0};incByType[e.type].pats[pn].a+=e.amount||0;incByType[e.type].pats[pn].comm+=getComm(e)})
      const incSplit=Object.entries(incByType).map(([t,d2])=>({t,label:(ITYPES.find(x=>x.key===t)||{}).full||t,amt:d2.amt,pats:Object.entries(d2.pats).map(([n,v])=>({n,a:v.a,pid:v.pid,isIP:v.isIP,comm:Math.round(v.comm||0)})).sort((x,y)=>y.a-x.a)})).filter(s=>s.amt>0).sort((a,b)=>b.amt-a.amt)
      const ipEntsColl=collInc.filter(e=>IP_T.includes(e.type))
      if(ipEntsColl.length>0){
        const byPatIP={}
        ipEntsColl.forEach(e=>{const pn=(e.patient_name||'—').trim()||'—';if(!byPatIP[pn])byPatIP[pn]={a:0,pid:null,isIP:true,types:{},comm:0};byPatIP[pn].a+=e.amount||0;byPatIP[pn].comm+=getComm(e);byPatIP[pn].types[e.type]=(byPatIP[pn].types[e.type]||0)+(e.amount||0);if(e.patient_id)byPatIP[pn].pid=e.patient_id})
        incSplit.push({t:'__ip__',label:'IP patients ('+Object.keys(byPatIP).length+')',
          amt:ipEntsColl.reduce((a,e)=>a+(e.amount||0),0),
          pats:Object.entries(byPatIP).map(([n,v])=>({n,a:v.a,pid:v.pid,isIP:true,comm:Math.round(v.comm||0),
    ...
    const Card1=({title,d,color})=>(<div style={{background:'#fff',border:'2px solid '+color.border,borderRadius:14,padding:'14px 16px'}}>
      ...
      <table style={{width:'100%',borderCollapse:'collapse',fontSize:13.5}}>
        <tbody>
          ...
          {(d.incSplit||[]).map(s=>(<Fragment key={s.t}><tr style={{fontSize:12,color:'#64748b'}}><td style={{padding:'1px 0 1px 10px'}}>↳ {s.label}</td><td style={{textAlign:'right',padding:'1px 0',color:'#16a34a'}}>{fmt(s.amt)}</td></tr>{(s.pats||[]).map(p=>(<Fragment key={s.t+p.n}><tr style={{fontSize:11,color:'#94a3b8'}}><td style={{padding:'0 0 0 20px'}}>· {(gotoOP||gotoIP)?<button onClick={()=>{if(p.isIP&&p.pid&&gotoIP)gotoIP(p.pid);else if(gotoOP)gotoOP(p.n)}} style={{background:'none',border:'none',padding:0,color:'#2563eb',fontSize:11,cursor:'pointer',textDecoration:'underline'}}>{p.n}</button>:p.n}{p.types&&<span style={{color:'#cbd5e1'}}> · {p.types}</span>}</td><td style={{textAlign:'right',padding:0,color:'#86efac'}}>{fmt(p.a)}</td></tr>{p.comm>0&&<tr style={{fontSize:10,color:'#c2410c'}}><td style={{padding:'0 0 2px 30px'}}>↳ after Dr. commission: {fmt(p.a)} − {fmt(p.comm)}</td><td style={{textAlign:'right',padding:'0 0 2px 0',fontWeight:700,color:'#0f766e'}}>= {fmt(p.a-p.comm)}</td></tr>}</Fragment>))}</Fragment>))}
...
const DailyDetailReport=({db,rd,setRd,allPaidComm,rm,setRm,ry,setRy,yrs,actions,gotoIP,gotoTimeline,gotoOP})=>{
  ...
  return(<>
    ...
    {/* OP CONSULTATION */}
    <SegmentPL db={db} gotoOP={n=>gotoOP&&gotoOP(n,'rep')} gotoIP={pid=>gotoIP&&gotoIP(pid,'rep')} incList={dI} expList={dExpAll} mtdIncList={db.income.filter(e=>e.date>=rd.slice(0,7)+'-01'&&e.date<=rd)} mtdExpList={db.expenses.filter(e=>e.date>=rd.slice(0,7)+'-01'&&e.date<=rd)} mtdLabel={fmtD(rd.slice(0,7)+'-01')+' → '+fmtD(rd)}/>
