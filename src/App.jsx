...
/*  IP BILLING MODULE  */
const AutoInput=({value,onChange,placeholder,suggestions})=>{
  return(<input value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder} style={{width:'100%',padding:'8px',border:'1px solid #e2e8f0',borderRadius:8,fontSize:13,outline:'none'}}/>)
}
const IPBillingModule=({p,db,onClose})=>{
  return(<div style={{padding:16}}><button onClick={onClose}>← Back</button><div style={{padding:20,textAlign:'center',color:'#888'}}>Billing module loading...</div></div>)
}
