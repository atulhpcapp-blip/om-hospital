...
const TABS   = [{k:'entry',l:'Daily Entry'},{k:'ip',l:'IP Patients'},{k:'exp',l:'Expenses'},{k:'rep',l:'Reports'}]
...
const LoginPage = ({onLogin}) => {
  ...
  const go = async () => {
    ...
    if(mode==='login'){
      ...
      if(error){setErr(error.message)}
        ...
        await supabase.from('profiles').upsert({id:data.user.id,name,role:'staff'})
  ...
  return (
    <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'linear-gradient(135deg,#f0f4ff 0%,#f7f7f7 100%)',padding:20}}>
      <div style={{width:'100%',maxWidth:380}}>
        <div style={{textAlign:'center',marginBottom:32}}>
          <div style={{fontSize:48,marginBottom:12}}>🏥</div>
          <div style={{fontSize:24,fontWeight:800,color:'#111'}}>Om Hospital</div>
          <div style={{fontSize:14,color:'#aaa',marginTop:4}}>Accounts & Finance System</div>
        </div>
        <Card>
          <div style={{display:'flex',gap:0,marginBottom:20,border:'1px solid #e5e7eb',borderRadius:10,overflow:'hidden'}}>
            {['login','signup'].map(m=>(
              <button key={m} onClick={()=>setMode(m)} style={{flex:1,padding:'10px',border:'none',background:mode===m?'#111':'#fff',color:mode===m?'#fff':'#888',fontWeight:600,fontSize:14,cursor:'pointer'}}>
                {m==='login'?'Login':'New Account'}
              </button>
            ))}
          </div>
          {mode==='signup'&&<FInp label="Your name" type="text" placeholder="Dr. / Sister / Staff name" value={name} onChange={e=>setName(e.target.value)}/>}
          <FInp label="Email" type="email" placeholder="your@email.com" value={email} onChange={e=>setEmail(e.target.value)}/>
          <FInp label="Password" type="password" placeholder="Password" value={pass} onChange={e=>setPass(e.target.value)}/>
          {err&&<div style={{fontSize:13,color:err.includes('created')?'#16a34a':'#ef4444',marginBottom:10,padding:'8px 12px',borderRadius:8,background:err.includes('created')?'#f0fdf4':'#fef2f2'}}>{err}</div>}
          <PBtn onClick={go} disabled={busy||!email||!pass}>{busy?'Please wait…':mode==='login'?'Login':'Create Account'}</PBtn>
        </Card>
        <div style={{textAlign:'center',fontSize:12,color:'#ccc',marginTop:16}}>Om Hospital · Secure login</div>
      </div>
    </div>
  )
}
    ...
    return(
      <div>
        ...
        {['ip','ip_r','ip_l'].map(tk=>{const te=ents.filter(e=>e.type===tk);if(!te.length)return null;const it=ITYPES.find(t=>t.key===tk);return(
          <div key={tk}>
            <SecL>{it.full} — {fmt(te.reduce((a,e)=>a+e.amount,0))}</SecL>
            <Card>{te.map(e=><Row key={e.id} left={fmtD(e.date)} sub={`${e.payment}${e.notes?' · '+e.notes:''} · Commission: ${fmt(getComm(e))}`} right={<><span style={{color:'#16a34a',fontWeight:600,fontSize:13}}>{fmt(e.amount)}</span><DBtn onClick={()=>actions.delIncome(e.id)}>✕</DBtn></>}/>)}</Card>
          </div>
        )})}
...
const RepTab = ({db,rv,setRv,rd,setRd,rm,setRm,ry,setRy,gotoIP}) => {
  ...
  const RefRep=({income})=>{
    ...
    return(<>
      <div style={{background:'#fff7ed',border:'1px solid #fed7aa',borderRadius:12,padding:'14px 16px',marginBottom:12}}>
        <div style={{fontSize:11,color:'#92400e',fontWeight:700,textTransform:'uppercase',letterSpacing:'.05em',marginBottom:4}}>Total commission payable</div>
        <div style={{fontSize:28,fontWeight:700,color:'#c2410c'}}>{fmt(tc)}</div>
        <div style={{fontSize:11,color:'#b45309',marginTop:2}}>{docs.length} doctor{docs.length>1?'s':''}</div>
      </div>
      {docs.map(doc=>(
        <Card key={doc.name}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:10}}>
            ...
            <div style={{textAlign:'right'}}><div style={{fontSize:11,color:'#d97706',fontWeight:600}}>Commission due</div><div style={{fontSize:22,fontWeight:700,color:'#c2410c'}}>{fmt(doc.total_commission)}</div></div>
          </div>
  ...
  return(
    <div>
      ...
      {rv==='daily'&&(()=>{
        const inc=sumInc(db.income.filter(e=>e.date===rd))
        const exp=sumExp(db.expenses.filter(e=>e.date===rd))
        const rc=totalRef(db.income.filter(e=>e.date===rd),db.ip_patients)
        const ipd=db.ip_patients.filter(p=>db.income.some(e=>e.patient_id===p.id&&e.date===rd))
        return(<>
          ...
          <PLCards inc={inc} exp={exp} refComm={rc}/>
          {ipd.length>0&&(<><SecL>IP activity</SecL><Card>{ipd.map(p=>{const t=db.income.filter(e=>e.patient_id===p.id&&e.date===rd).reduce((a,e)=>a+e.amount,0);const tot=db.income.filter(e=>e.patient_id===p.id).reduce((a,e)=>a+e.amount,0);const pd=(p.payments||[]).reduce((a,e)=>a+e.amount,0);return<Row key={p.id} left={p.name} sub={`Today: ${fmt(t)} · Total: ${fmt(tot)}`} right={tot-pd>0?<span style={{color:'#ef4444',fontSize:11,fontWeight:600}}>due {fmt(tot-pd)}</span>:<span style={{color:'#16a34a',fontSize:11}}>settled</span>} onClick={()=>gotoIP(p.id)}/>})}</Card></>)}
          <SecL>Income by source</SecL><IncT inc={inc}/>
      ...
      {rv==='monthly'&&(()=>{
        ...
        const inc=sumInc(mI);const exp=sumExp(mE);const rc=totalRef(mI,db.ip_patients)
      ...
      {rv==='yearly'&&(()=>{
        ...
        const inc=sumInc(yI);const exp=sumExp(yE);const rc=totalRef(yI,db.ip_patients)
        ...
        return(<>
          ...
          {mons.length>0&&(<><SecL>Month-wise</SecL><Card>
            <div style={{display:'grid',gridTemplateColumns:'auto 1fr 1fr 1fr',marginBottom:4}}>
              {['Month','Revenue','Exp','Net'].map(h=><div key={h} style={{fontSize:10,color:'#aaa',fontWeight:700,textTransform:'uppercase',padding:'4px 4px 8px 0',borderBottom:'1px solid #f0f0f0'}}>{h}</div>)}
            </div>
            {mons.map(ym=>{const mi=db.income.filter(e=>e.date?.startsWith(ym)).reduce((a,e)=>a+e.amount,0);const me=db.expenses.filter(e=>e.date?.startsWith(ym)).reduce((a,e)=>a+e.amount,0);const mc=totalRef(db.income.filter(e=>e.date?.startsWith(ym)),db.ip_patients);const mn=mi-me-mc;const[,m]=ym.split('-');return(
              <div key={ym} onClick={()=>{setRv('monthly');setRm(ym)}} style={{display:'grid',gridTemplateColumns:'auto 1fr 1fr 1fr',padding:'8px 0',borderBottom:'1px solid #f5f5f5',cursor:'pointer'}}>
                <span style={{fontSize:13,paddingRight:8}}>{MOS[parseInt(m)-1]}</span>
                <span style={{fontSize:13,color:'#16a34a',fontWeight:600}}>{fmt(mi)}</span>
                <span style={{fontSize:13,color:'#ef4444'}}>{fmt(me)}</span>
                <span style={{fontSize:13,color:mn>=0?'#16a34a':'#ef4444',fontWeight:600}}>{mn>=0?'+':''}{fmt(mn)}</span>
              </div>
            )})}
          </Card></>)}
          <SecL>Income by source</SecL><IncT inc={inc}/>
          <SecL>Doctor referral report — {ry}</SecL><RefRep income={yI}/>
        </>)
      })()}

      {rv==='referrals'&&(()=>{
        const [refPer,setRefPer]=useState('month')
        const fi=refPer==='month'?db.income.filter(e=>e.date?.startsWith(rm)):db.income.filter(e=>e.date?.startsWith(ry))
        return(<>
...
export default function App() {
  ...
  const actions = {
    addIncome: async row => {
      const {data}=await supabase.from('income').insert([row]).select()
      if(data)setDb(d=>({...d,income:[data[0],...d.income]}))
    },
    delIncome: async id => {
    ...
    admitPatient: async row => {
      const {data}=await supabase.from('ip_patients').insert([row]).select()
      if(data)setDb(d=>({...d,ip_patients:[data[0],...d.ip_patients]}))
    },
    dischargePatient: async id => {
      const {data}=await supabase.from('ip_patients').update({discharge_date:todayStr()}).eq('id',id).select()
      if(data)setDb(d=>({...d,ip_patients:d.ip_patients.map(p=>p.id===id?data[0]:p)}))
    },
    addPayment: async (pid, payment) => {
      const p=db.ip_patients.find(x=>x.id===pid)
      const newPayments=[...(p.payments||[]),payment]
      const {data}=await supabase.from('ip_patients').update({payments:newPayments}).eq('id',pid).select()
      if(data)setDb(d=>({...d,ip_patients:d.ip_patients.map(x=>x.id===pid?data[0]:x)}))
    },
  }
    ...
    <div style={{maxWidth:520,margin:'0 auto',background:'#f7f7f7',minHeight:'100vh'}}>
      ...
      <div style={{background:'#fff',borderBottom:'1px solid #f0f0f0',padding:'12px 16px 0',position:'sticky',top:0,zIndex:10}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
          <div style={{fontWeight:700,fontSize:15,color:'#111'}}>🏥 Om Hospital Accounts</div>
          <button onClick={()=>supabase.auth.signOut()} style={{fontSize:12,color:'#aaa',background:'none',border:'none',cursor:'pointer'}}>Logout</button>
        </div>
        {dbLoading&&<div style={{fontSize:11,color:'#aaa',marginBottom:6,textAlign:'center'}}>Loading data…</div>}
        <div style={{display:'flex',overflowX:'auto',gap:0,marginBottom:-1}}>
          {TABS.map(t=>(
            <button key={t.k} onClick={()=>setTab(t.k)} style={{flexShrink:0,padding:'9px 12px',fontSize:13,fontWeight:600,border:'none',background:'none',color:tab===t.k?'#111':'#bbb',borderBottom:tab===t.k?'2.5px solid #111':'2.5px solid transparent',cursor:'pointer',whiteSpace:'nowrap'}}>
              {t.l}
            </button>
          ))}
        </div>
