              ...
              return(<div key={e.id} style={{marginBottom:8,padding:'8px 10px',background:'#f9fafb',borderRadius:10,border:'1px solid #f0f0f0'}}>
                {!isEditing
                  ...
                  :<div>
                    ...
                    <div style={{display:'flex',gap:8}}>
                      <button onClick={async()=>{const amt=parseFloat(editPayForm.amount);if(!amt||amt<=0){alert('Enter valid amount');return};await supabase.from('expenses').update({amount:amt,date:editPayForm.date,payment:editPayForm.payment}).eq('id',e.id);setEditPayId(null);window.location.reload()}} style={{flex:1,padding:'9px',background:'#16a34a',color:'#fff',border:'none',borderRadius:9,fontSize:13,fontWeight:700,cursor:'pointer'}}>Save</button>
