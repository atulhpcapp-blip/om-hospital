...
const DailyDetailReport=({db,rd,setRd,allPaidComm,rm,setRm,ry,setRy,yrs,actions,gotoIP,gotoTimeline})=>{
  ...
  // OP Consultation (op) — grouped by patient
  const opEnts=dI.filter(e=>e.type==='op')
  const opByPat={}
  opEnts.forEach(e=>{const k=(e.patient_name||'Unknown').trim().toLowerCase();if(!opByPat[k])opByPat[k]={name:(e.patient_name||'Unknown').trim(),patient_id:e.patient_id,entries:[]};opByPat[k].entries.push(e)})

  // OP Pharmacy (op_r) — grouped by patient
  const oprEnts=dI.filter(e=>e.type==='op_r')
  const oprByPat={}
  oprEnts.forEach(e=>{const k=(e.patient_name||'Unknown').trim().toLowerCase();if(!oprByPat[k])oprByPat[k]={name:(e.patient_name||'Unknown').trim(),patient_id:e.patient_id,entries:[]};oprByPat[k].entries.push(e)})

  // VC (visiting consultants)
  const vcEnts=dI.filter(e=>e.type==='vc')

  // IP income — grouped by patient (ip + ip_r only, no lab)
  const ipEnts=dI.filter(e=>['ip','ip_r'].includes(e.type))
  const ipByPat={}
  ipEnts.forEach(e=>{const k=e.patient_id||e.patient_name||'?';const name=(e.patient_name||'Unknown').trim();if(!ipByPat[k])ipByPat[k]={id:e.patient_id,name,ip:0,ip_r:0,ref:''};if(e.type==='ip')ipByPat[k].ip+=e.amount;if(e.type==='ip_r')ipByPat[k].ip_r+=e.amount;if(e.ref_doctor&&!ipByPat[k].ref)ipByPat[k].ref=e.ref_doctor})

  // Lab — deduplicated by patient+type
  const opLabSeen=new Set()
  const opLabEnts=dI.filter(e=>{if(e.type!=='op_l')return false;const key=(e.patient_name||'').trim().toLowerCase()+'_'+e.amount;if(opLabSeen.has(key))return false;opLabSeen.add(key);return true})
  const ipLabSeen=new Set()
  const ipLabEnts=dI.filter(e=>{if(e.type!=='ip_l')return false;const key=(e.patient_id||e.patient_name||'').toLowerCase()+'_'+e.amount;if(ipLabSeen.has(key))return false;ipLabSeen.add(key);return true})
  const labInc=dI.filter(e=>['op_l','ip_l'].includes(e.type)).reduce((a,e)=>a+e.amount,0)
  const labComm=dI.filter(e=>['op_l','ip_l'].includes(e.type)).reduce((a,e)=>a+getComm(e),0)
  const labToLab=dExp.filter(e=>e.category==='lab_to_lab').reduce((a,e)=>a+e.amount,0)
  const labActual=labInc-labComm-labToLab

  // Clinical & Pharmacy segment (op + op_r + ip + ip_r)
  ...
  const opConsActual=opConsInc-opConsComm

  // Segment: OP Pharmacy only
  ...
  const opPharmaActual=opPharmaInc-opPharmaComm

  const R=({l,v,bold,red,green})=>(<div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'6px 0',borderBottom:'1px solid #f5f5f5'}}><span style={{fontSize:13,color:'#374151',fontWeight:bold?700:400}}>{l}</span><span style={{fontSize:13,fontWeight:bold?800:600,color:red?'#dc2626':green?'#16a34a':'#0f172a'}}>{v}</span></div>)

  const NameBtn=({name,pid,type})=>{
    const ipPat=pid?db.ip_patients.find(p=>p.id===pid):null
    const handleClick=()=>{
      if(type==='ip'&&ipPat&&gotoIP){gotoIP(ipPat.id)}
      else if(gotoTimeline&&ipPat){gotoTimeline(ipPat.id)}
    }
    return(<button onClick={handleClick} style={{fontSize:13,fontWeight:700,color:type==='ip'?'#2563eb':'#1d4ed8',background:'none',border:'none',cursor:gotoIP||gotoTimeline?'pointer':'default',padding:0,textAlign:'left',textDecoration:'underline',textDecorationColor:'rgba(37,99,235,0.3)'}}>{name}</button>)
  }
  ...
  return(<>
    ...
    {opEnts.length===0&&vcEnts.length===0?<div style={{color:'#ccc',fontSize:13,padding:'8px 0',marginBottom:8}}>No OP consultations</div>:<Card>
      {Object.values(opByPat).map(pat=>{
        ...
        const ipPat=pat.patient_id?db.ip_patients.find(p=>p.id===pat.patient_id):db.ip_patients.find(p=>p.name.trim().toLowerCase()===pat.name.toLowerCase())
        return(<div key={pat.name} style={{padding:'9px 0',borderBottom:'1px solid #f5f5f5'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
            <div style={{flex:1}}>
              <NameBtn name={pat.name} pid={ipPat?.id} type="op"/>
              {ref&&<div style={{fontSize:11,color:'#d97706',marginTop:2}}>Ref: {ref}</div>}
              {consFee>0&&<div style={{fontSize:11,color:'#7c3aed',marginTop:2}}>Cons fee paid: {fmt(consFee)}{consName?' ('+consName+')':''}</div>}
            </div>
      ...
      {vcEnts.map((e,i)=>(<div key={i} style={{padding:'9px 0',borderBottom:'1px solid #f5f5f5'}}>
        <div style={{display:'flex',justifyContent:'space-between'}}>
          <div><NameBtn name={e.patient_name||'Patient'} pid={e.patient_id} type="op"/><div style={{fontSize:11,color:'#7c3aed',marginTop:2}}>VC: {e.consultant_name||''} — Fee: {fmt(e.consultant_fee||0)}</div></div>
          <div style={{textAlign:'right'}}><div style={{fontSize:14,fontWeight:700,color:'#16a34a'}}>{fmt(e.amount)}</div><TypeTag t="vc"/></div>
        </div>
      </div>))}
      <R l="OP Consultation Total" v={fmt(opConsInc+vcEnts.reduce((a,e)=>a+e.amount,0))} bold green/>
    </Card>}

    {/* OP PHARMACY */}
    <SecL>OP Pharmacy</SecL>
    {oprEnts.length===0?<div style={{color:'#ccc',fontSize:13,padding:'8px 0',marginBottom:8}}>No OP pharmacy entries</div>:<Card>
      {Object.values(oprByPat).map(pat=>{
        ...
        return(<div key={pat.name} style={{padding:'9px 0',borderBottom:'1px solid #f5f5f5'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
            <div><NameBtn name={pat.name} pid={ipPat?.id} type="op"/>{ref&&<div style={{fontSize:11,color:'#d97706',marginTop:2}}>Ref: {ref}</div>}</div>
    ...
    {Object.keys(ipByPat).length===0?<div style={{color:'#ccc',fontSize:13,padding:'8px 0',marginBottom:8}}>No IP entries today</div>:<Card>
      {Object.values(ipByPat).map(pat=>{
        const ipPat=pat.id?db.ip_patients.find(p=>p.id===pat.id):null
        return(<div key={pat.id||pat.name} style={{padding:'9px 0',borderBottom:'1px solid #f5f5f5'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
            <div style={{flex:1}}>
              <NameBtn name={pat.name} pid={ipPat?.id} type="ip"/>
    ...
    {opLabEnts.length===0&&ipLabEnts.length===0?<div style={{color:'#ccc',fontSize:13,padding:'8px 0',marginBottom:8}}>No lab entries today</div>:<Card>
      {opLabEnts.length>0&&<div style={{marginBottom:10}}>
        ...
        {opLabEnts.map((e,i)=>{
          const ipPat=e.patient_id?db.ip_patients.find(p=>p.id===e.patient_id):null
          return(<div key={i} style={{display:'flex',justifyContent:'space-between',padding:'6px 0',borderBottom:'1px solid #f5f5f5'}}>
            <div><NameBtn name={e.patient_name||'Patient'} pid={ipPat?.id} type="op"/>{e.ref_doctor&&<div style={{fontSize:11,color:'#d97706'}}>Ref: {e.ref_doctor}</div>}</div>
      ...
      {ipLabEnts.length>0&&<div>
        ...
        {ipLabEnts.map((e,i)=>{
          const ipPat=e.patient_id?db.ip_patients.find(p=>p.id===e.patient_id):null
          return(<div key={i} style={{display:'flex',justifyContent:'space-between',padding:'6px 0',borderBottom:'1px solid #f5f5f5'}}>
            <div><NameBtn name={e.patient_name||'Patient'} pid={ipPat?.id} type="ip"/>{e.ref_doctor&&<div style={{fontSize:11,color:'#d97706'}}>Ref: {e.ref_doctor}</div>}</div>
    ...
    {/* EXPENSES */}
    <SecL>Expenses</SecL>
    {dExp.length===0?<div style={{color:'#ccc',fontSize:13,padding:'8px 0',marginBottom:8}}>No expenses today</div>:<Card>
      {dExp.map((e,i)=>(<div key={i} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'7px 0',borderBottom:'1px solid #f5f5f5'}}>
    ...
    <div style={{display:'flex',flexDirection:'column',gap:12,marginBottom:16}}>
      {/* OP Consultation segment */}
      <div style={{background:'#f0fdf4',border:'1px solid #bbf7d0',borderRadius:14,padding:'14px'}}>
        <div style={{display:'flex',justifyContent:'space-between',marginBottom:10}}>
          <div><div style={{fontSize:13,fontWeight:700,color:'#15803d'}}>OP Consultation</div><div style={{fontSize:10,color:'#86efac'}}>OP + VC entries</div></div>
          <div style={{textAlign:'right'}}><div style={{fontSize:10,color:'#86efac'}}>Actual</div><div style={{fontSize:20,fontWeight:800,color:opConsActual>=0?'#15803d':'#dc2626'}}>{fmt(opConsActual)}</div></div>
        </div>
        <div style={{background:'rgba(255,255,255,0.7)',borderRadius:8,padding:'8px 10px',display:'flex',flexDirection:'column',gap:5}}>
          <R l="OP income" v={fmt(opConsInc)} green/>
          {opConsComm>0&&<R l="Ref commissions" v={'- '+fmt(opConsComm)} red/>}
          <div style={{height:1,background:'#bbf7d0'}}/>
          <R l="= Actual" v={fmt(opConsActual)} bold/>
        </div>
      </div>
      {/* OP Pharmacy segment */}
      <div style={{background:'#f0fdf4',border:'1px solid #bbf7d0',borderRadius:14,padding:'14px'}}>
        <div style={{display:'flex',justifyContent:'space-between',marginBottom:10}}>
          <div><div style={{fontSize:13,fontWeight:700,color:'#15803d'}}>OP Pharmacy</div><div style={{fontSize:10,color:'#86efac'}}>OP-Pharmacy entries</div></div>
          <div style={{textAlign:'right'}}><div style={{fontSize:10,color:'#86efac'}}>Actual</div><div style={{fontSize:20,fontWeight:800,color:opPharmaActual>=0?'#15803d':'#dc2626'}}>{fmt(opPharmaActual)}</div></div>
        </div>
        <div style={{background:'rgba(255,255,255,0.7)',borderRadius:8,padding:'8px 10px',display:'flex',flexDirection:'column',gap:5}}>
          <R l="Pharmacy income" v={fmt(opPharmaInc)} green/>
          {opPharmaComm>0&&<R l="Ref commissions" v={'- '+fmt(opPharmaComm)} red/>}
          <div style={{height:1,background:'#bbf7d0'}}/>
          <R l="= Actual" v={fmt(opPharmaActual)} bold/>
        </div>
      </div>
      {/* Clinical & Pharmacy combined */}
      <div style={{background:'#f0f9ff',border:'1px solid #bae6fd',borderRadius:14,padding:'14px'}}>
        <div style={{display:'flex',justifyContent:'space-between',marginBottom:10}}>
          <div><div style={{fontSize:13,fontWeight:700,color:'#0369a1'}}>Clinical and Pharmacy</div><div style={{fontSize:10,color:'#7dd3fc'}}>OP + Pharmacy + IP + IP-Pharmacy — all non-lab expenses</div></div>
          <div style={{textAlign:'right'}}><div style={{fontSize:10,color:'#7dd3fc'}}>Actual</div><div style={{fontSize:20,fontWeight:800,color:clinActual>=0?'#0369a1':'#dc2626'}}>{fmt(clinActual)}</div></div>
        </div>
        <div style={{background:'rgba(255,255,255,0.7)',borderRadius:8,padding:'8px 10px',display:'flex',flexDirection:'column',gap:5}}>
          <R l="Total income" v={fmt(clinInc)} green/>
          <R l="Ref commissions" v={'- '+fmt(clinComm)} red/>
          {nonLabExp.map((e,i)=>(<R key={i} l={(e.category||'misc').replace(/_/g,' ')} v={'- '+fmt(e.amount)} red/>))}
          <div style={{height:1,background:'#bae6fd'}}/>
          <R l="= Actual" v={fmt(clinActual)} bold/>
        </div>
      </div>
      {/* Lab segment */}
      <div style={{background:'#fdf4ff',border:'1px solid #e9d5ff',borderRadius:14,padding:'14px'}}>
        <div style={{display:'flex',justifyContent:'space-between',marginBottom:10}}>
          <div><div style={{fontSize:13,fontWeight:700,color:'#7c3aed'}}>Laboratory</div><div style={{fontSize:10,color:'#c4b5fd'}}>OP-Lab + IP-Lab minus lab-to-lab expenses</div></div>
          <div style={{textAlign:'right'}}><div style={{fontSize:10,color:'#c4b5fd'}}>Actual</div><div style={{fontSize:20,fontWeight:800,color:labActual>=0?'#7c3aed':'#dc2626'}}>{fmt(labActual)}</div></div>
        </div>
        <div style={{background:'rgba(255,255,255,0.7)',borderRadius:8,padding:'8px 10px',display:'flex',flexDirection:'column',gap:5}}>
          <R l="Lab income" v={fmt(labInc)} green/>
          {labComm>0&&<R l="Ref commissions" v={'- '+fmt(labComm)} red/>}
          {labToLab>0&&<R l="Lab to lab" v={'- '+fmt(labToLab)} red/>}
          <div style={{height:1,background:'#e9d5ff'}}/>
          <R l="= Actual" v={fmt(labActual)} bold/>
        </div>
      </div>
    </div>
...
const RepTab=({db,rv,setRv,rd,setRd,rm,setRm,ry,setRy,gotoIP,actions})=>{
  ...
  return(
    <div>
      ...
      {rv==='daily'&&<DailyDetailReport db={db} rd={rd} setRd={setRd} allPaidComm={allPaidComm} rm={rm} setRm={setRm} ry={ry} setRy={setRy} yrs={yrs} actions={actions} gotoIP={gotoIP} gotoTimeline={pid=>{setTimelineSelPid(pid);setRv('timeline')}}/>}
