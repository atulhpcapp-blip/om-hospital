    ...
    return(
      <div>
        ...
        {(()=>{
          ...
          const pfx=per==='month'?rm:ry
          const _exp=db.expenses.filter(e=>e.category!=='ref_paid'&&e.date?.startsWith(pfx)).reduce((a,e)=>a+e.amount,0)
...
const RealIncomeReport=({db})=>{
  ...
  const rExpList=(db.expenses||[]).filter(e=>e.category!=='ref_paid'&&(()=>{if(rPer==='day')return e.date===rDay;if(rPer==='month')return e.date?.startsWith(rMon);if(rPer==='year')return e.date?.startsWith(rYr);if(rPer==='custom')return e.date>=rFrom&&e.date<=rTo;return true})())
  return(<>
    ...
    {!hasData
      ...
      :<>
        <Card>
          <div style={{display:'grid',gridTemplateColumns:'1fr 70px 70px 70px',gap:2,marginBottom:8,paddingBottom:8,borderBottom:'1px solid #f0f0f0'}}>
            <div style={{fontSize:9,color:'#aaa',fontWeight:700,textTransform:'uppercase'}}>Category</div>
            <div style={{fontSize:9,color:'#aaa',fontWeight:700,textTransform:'uppercase',textAlign:'right'}}>Coll.</div>
...
const DailyDetailReport=({db,rd,setRd,allPaidComm,rm,setRm,ry,setRy,yrs,actions,gotoIP,gotoTimeline,gotoOP})=>{
  ...
  return(<>
    ...
    {(()=>{
      ...
      const _exp=dExpAll.reduce((a,e)=>a+e.amount,0)
      const _actual=_coll-_comm-_vcFee
      const _real=_actual-_exp
      return(
        <div style={{background:'linear-gradient(135deg,#0f172a 0%,#1e293b 100%)',borderRadius:16,padding:'16px',marginBottom:14,color:'#fff'}}>
