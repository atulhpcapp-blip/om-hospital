...
const DeductCommForm=({docName,balance,db,onSave,onCancel})=>{
  ...
  const total=(parseFloat(clin)||0)+(parseFloat(lab)||0)
  const fillProportional=()=>{
    const tot=clEarned+lbEarned
    const cl=tot>0?Math.round(balance*clEarned/tot):Math.round(balance)
    setClin(String(cl));setLab(String(Math.round(balance)-cl))
  }
  const go=async()=>{
    const ca=parseFloat(clin)||0,la=parseFloat(lab)||0
    if(ca<=0&&la<=0){alert('Enter deduction amount for Clinical and/or Lab');return}
    if((ca+la)>balance+0.5){alert('Total deduction '+fmt(ca+la)+' cannot be more than the current due '+fmt(balance)+'.\n\nDeducting beyond the due would inflate profit. Reduce the amounts.');return}
    setBusy(true);await onSave(ca,la,date);setBusy(false)
  }
  return(
    <div style={{background:'#fffbeb',borderRadius:10,padding:'12px 14px',border:'1px solid #fde68a',marginTop:10}}>
      <div style={{fontSize:12,fontWeight:700,color:'#92400e',marginBottom:4}}>Deduct commission — Dr. {docName}</div>
      <div style={{fontSize:11,color:'#a16207',marginBottom:8}}>Reduces due and adjusts profit per segment. No money is paid — accounts adjustment only.</div>
      <div style={{fontSize:10.5,color:'#a16207',marginBottom:8,background:'#fef3c7',borderRadius:6,padding:'5px 8px'}}>Doctor's earned split — 🏥 Clinical: <b>{fmt(clEarned)}</b> · 🧪 Lab: <b>{fmt(lbEarned)}</b> <button onClick={fillProportional} style={{marginLeft:6,padding:'2px 8px',background:'#d97706',color:'#fff',border:'none',borderRadius:6,fontSize:10,fontWeight:700,cursor:'pointer'}}>Auto-split {fmt(balance)}</button></div>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:8}}>
        <div><label style={{display:'block',fontSize:9,color:'#a16207',fontWeight:700,textTransform:'uppercase',marginBottom:3}}>🏥 From Clinical (Rs)</label><input type="number" inputMode="numeric" value={clin} onChange={e=>setClin(e.target.value)} placeholder="0" style={{width:'100%',padding:'9px 10px',border:'1.5px solid #fcd34d',borderRadius:8,fontSize:14,fontWeight:700,boxSizing:'border-box',outline:'none'}}/></div>
        <div><label style={{display:'block',fontSize:9,color:'#a16207',fontWeight:700,textTransform:'uppercase',marginBottom:3}}>🧪 From Lab (Rs)</label><input type="number" inputMode="numeric" value={lab} onChange={e=>setLab(e.target.value)} placeholder="0" style={{width:'100%',padding:'9px 10px',border:'1.5px solid #fcd34d',borderRadius:8,fontSize:14,fontWeight:700,boxSizing:'border-box',outline:'none'}}/></div>
      </div>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:10,alignItems:'end'}}>
        <div><label style={{display:'block',fontSize:9,color:'#a16207',fontWeight:700,textTransform:'uppercase',marginBottom:3}}>Date</label><input type="date" value={date} onChange={e=>setDate(e.target.value)} style={{width:'100%',padding:'9px 10px',border:'1.5px solid #fcd34d',borderRadius:8,fontSize:13,boxSizing:'border-box',outline:'none'}}/><div style={{fontSize:9.5,color:'#a16207',marginTop:3,lineHeight:1.4}}>Tip: date it when the commission arose (e.g. discharge date) so daily reports line up.</div></div>
        <div style={{fontSize:12,fontWeight:800,color:'#92400e',paddingBottom:9}}>Total: {fmt(total)}</div>
      </div>
      <div style={{display:'flex',gap:8}}>
        ...
        <button onClick={go} disabled={busy} style={{flex:2,padding:'9px',background:'#d97706',color:'#fff',border:'none',borderRadius:8,fontSize:12,fontWeight:800,cursor:busy?'not-allowed':'pointer'}}>{busy?'Saving...':'− Deduct from due'}</button>
      </div>
    </div>)
}
        ...
        {(()=>{
          ...
          return(<div style={{marginTop:10}}>
            ...
            {payDocI==='PAY'?<CommPayForm docName={dn} balance={Math.max(0,Math.min(b.commission,gDue))||b.commission} onCancel={()=>setPayDocI(null)} onSave={async(amt,date,pay)=>{await settleRefPayment(db,actions,dn,amt,date,pay,0);setPayDocI(null)}}/>
             :payDocI==='DED'?<DeductCommForm db={db} docName={dn} balance={Math.max(0,gDue)} onCancel={()=>setPayDocI(null)} onSave={async(clinAmt,labAmt,date)=>{await deductCommSplit(actions,dn,date,clinAmt,labAmt);setPayDocI(null)}}/>
  ...
  if(selPat){
    ...
    return(
      <div>
        ...
        :payDoc==='DED:'+doc.name?<DeductCommForm db={db} docName={doc.name} balance={balance} onCancel={()=>setPayDoc(null)} onSave={async(clinAmt,labAmt,date)=>{await deductCommSplit(actions,doc.name,date,clinAmt,labAmt);setPayDoc(null)}}/>
        :<div style={{display:'flex',gap:8}}>
      ...
      return(<>
        ...
        {dueRows.map(r=>{const isOpen=payDoc==='DUE:'+r.name;return(<Card key={r.name} style={{border:'1px solid #fed7aa'}}>
          ...
          {payDoc==='DUE:'+r.name?<CommPayForm docName={r.name} balance={r.due} onCancel={()=>setPayDoc(null)} onSave={async(amt,date,pay)=>{await settleRefPayment(db,actions,r.name,amt,date,pay,0);setPayDoc(null)}}/>
           :payDoc==='DED:'+r.name?<DeductCommForm db={db} docName={r.name} balance={r.due} onCancel={()=>setPayDoc(null)} onSave={async(clinAmt,labAmt,date)=>{await deductCommSplit(actions,r.name,date,clinAmt,labAmt);setPayDoc(null)}}/>
           :<div style={{display:'flex',gap:8}}>
      ...
      :<>
        ...
        {clinGross>0&&<div style={{background:'#fff',border:'1px solid #f0f0f0',borderRadius:16,padding:'16px',marginBottom:12}}>
          ...
          <div style={{background:'#f8fafc',borderRadius:10,padding:'10px 12px',display:'flex',flexDirection:'column',gap:6}}>
            ...
            {riRetC>0&&<><div style={{display:'flex',justifyContent:'space-between',fontSize:12}}><span style={{color:'#475569'}}>Commission retained</span><span style={{fontWeight:700,color:'#16a34a'}}>+ {fmt(riRetC)}</span></div>{Object.entries(expList.filter(e=>isRetainedCat(e.category)).reduce((m,e)=>{const n=(e.description||'(unknown)').trim()||'(unknown)';m[n]=(m[n]||0)+e.amount;return m},{})).sort((a,b)=>b[1]-a[1]).map(([n,amt])=>(<div key={n} style={{display:'flex',justifyContent:'space-between',fontSize:11,color:'#94a3b8',paddingLeft:12}}><span>↳ Dr. {n}</span><span style={{color:'#16a34a'}}>+ {fmt(Math.round(amt))}</span></div>))}</>}
...
/*  DAILY DETAIL REPORT  */
const DatewiseNetCard=({incList,expList})=>{
  ...
  const calc=(dayInc,dayExp)=>{
    const seg=(s)=>{
      ...
      const billed=si.filter(e=>e.payment!=='discount'&&e.payment!=='written_off').reduce((a,e)=>a+(e.amount||0),0)
...
const SegmentPL=({incList,expList,mtdIncList=null,mtdExpList=null,mtdLabel='',mtdTitle='Month to date net profit',monthlyOf=null})=>{
    ...
    const calcSeg=(seg,iL,eL)=>{
      ...
      const incByType={};sInc.filter(e=>e.payment!=='discount'&&e.payment!=='written_off').forEach(e=>{if(!incByType[e.type])incByType[e.type]={amt:0,pats:{}};incByType[e.type].amt+=e.amount||0;const pn=(e.patient_name||'—').trim()||'—';incByType[e.type].pats[pn]=(incByType[e.type].pats[pn]||0)+(e.amount||0)})
      ...
      const commByDoc={};sInc.forEach(e=>{const cm=getComm(e);if(cm>0&&e.ref_doctor){if(!commByDoc[e.ref_doctor])commByDoc[e.ref_doctor]={amt:0,pats:{}};commByDoc[e.ref_doctor].amt+=cm;const pn=(e.patient_name||'—').trim()||'—';commByDoc[e.ref_doctor].pats[pn]=(commByDoc[e.ref_doctor].pats[pn]||0)+cm}})
      ...
      const retRows=srcExp.filter(e=>e.category===(seg==='lab'?'comm_retained_lab':'comm_retained_clinical'))
      const retained=retRows.reduce((a,e)=>a+(e.amount||0),0)
      const retByDoc={};retRows.forEach(e=>{const n=(e.description||'(unknown)').trim()||'(unknown)';retByDoc[n]=(retByDoc[n]||0)+(e.amount||0)})
      const retSplit=Object.entries(retByDoc).map(([n,amt])=>{const cs=commSplit.find(s=>s.name===n);return{name:n,amt:Math.round(amt),comm:cs?cs.amt:0,pats:cs?cs.pats:[]}}).sort((a,b)=>b.amt-a.amt)
    ...
    const Card1=({title,d,color})=>(<div style={{background:'#fff',border:'2px solid '+color.border,borderRadius:14,padding:'14px 16px'}}>
      ...
      <table style={{width:'100%',borderCollapse:'collapse',fontSize:13.5}}>
        <tbody>
          ...
          {(d.retained||0)>0&&<><tr><td style={{padding:'4px 0',color:'#166534',fontWeight:600}}>Commission retained</td><td style={{textAlign:'right',padding:'4px 0',fontWeight:700,color:'#16a34a'}}>+{fmt(d.retained)}</td></tr>{(d.retSplit||[]).map(s=>(<Fragment key={s.name}><tr style={{fontSize:12,color:'#64748b'}}><td style={{padding:'1px 0 1px 10px'}}>↳ Dr. {s.name}{s.comm>0?<span style={{color:'#b45309'}}> · comm {fmt(s.comm)}</span>:null}</td><td style={{textAlign:'right',padding:'1px 0',color:'#16a34a'}}>+{fmt(s.amt)}</td></tr>{(s.pats||[]).map(pp=>(<tr key={pp.n} style={{fontSize:11,color:'#94a3b8'}}><td style={{padding:'0 0 0 20px'}}>· {pp.n}</td><td style={{textAlign:'right',padding:0,color:'#b45309'}}>comm {fmt(pp.a)}</td></tr>))}</Fragment>))}</>}
    ...
    <div style={{display:'flex',flexDirection:'column',gap:10,marginBottom:16}}>
      ...
      <div style={{background:'#f0f9ff',border:'1px solid #bae6fd',borderRadius:14,padding:'16px'}}>
        ...
        <div style={{background:'rgba(255,255,255,0.8)',borderRadius:10,padding:'10px 12px',display:'flex',flexDirection:'column',gap:5}}>
          ...
          {dRetainedClin>0&&<>{<R l="Commission retained (adjustment)" v={'+ '+fmt(dRetainedClin)} green sub="Deduction recorded today — may relate to earlier days' commissions"/>}{Object.entries(dExpAll.filter(e=>e.category==='comm_retained_clinical').reduce((m,e)=>{const n=(e.description||'(unknown)').trim()||'(unknown)';m[n]=(m[n]||0)+e.amount;return m},{})).sort((a,b)=>b[1]-a[1]).map(([n,amt])=>(<div key={n} style={{display:'flex',justifyContent:'space-between',fontSize:11,color:'#94a3b8',padding:'2px 0 2px 14px'}}><span>↳ Dr. {n}</span><span style={{color:'#16a34a'}}>+ {fmt(Math.round(amt))}</span></div>))}</>}{opIpConsFee>0&&<>{<R l="Consultant fees" v={'- '+fmt(opIpConsFee)} red sub="Accrued on today's entries"/>}{Object.entries(clinEntsAll.filter(e=>e.type!=='vc').reduce((m,e)=>{const cm=(e.consultant_fee||0);if(cm>0&&e.consultant_name){m[e.consultant_name]=(m[e.consultant_name]||0)+cm}return m},{})).sort((a,b)=>b[1]-a[1]).map(([n,amt])=>(<div key={n} style={{display:'flex',justifyContent:'space-between',fontSize:11,color:'#94a3b8',padding:'2px 0 2px 14px'}}><span>↳ Dr. {n}</span><span>- {fmt(Math.round(amt))}</span></div>))}</>}
      ...
      <div style={{background:'#fdf4ff',border:'1px solid #e9d5ff',borderRadius:14,padding:'16px'}}>
        ...
        <div style={{background:'rgba(255,255,255,0.8)',borderRadius:10,padding:'10px 12px',display:'flex',flexDirection:'column',gap:5}}>
          ...
          <>{dRetainedLab>0&&<>{<R l="Commission retained (adjustment)" v={'+ '+fmt(dRetainedLab)} green sub="Deduction recorded today — may relate to earlier days' commissions"/>}{Object.entries(dExpAll.filter(e=>e.category==='comm_retained_lab').reduce((m,e)=>{const n=(e.description||'(unknown)').trim()||'(unknown)';m[n]=(m[n]||0)+e.amount;return m},{})).sort((a,b)=>b[1]-a[1]).map(([n,amt])=>(<div key={n} style={{display:'flex',justifyContent:'space-between',fontSize:11,color:'#94a3b8',padding:'2px 0 2px 14px'}}><span>↳ Dr. {n}</span><span style={{color:'#16a34a'}}>+ {fmt(Math.round(amt))}</span></div>))}</>}<R l="Ref commissions" v={'- '+fmt(labComm)} red/>{Object.entries(labRawEnts.reduce((m,e)=>{const cm=getComm(e);if(cm>0&&e.ref_doctor){m[e.ref_doctor]=(m[e.ref_doctor]||0)+cm}return m},{})).sort((a,b)=>b[1]-a[1]).map(([n,amt])=>(<div key={n} style={{display:'flex',justifyContent:'space-between',fontSize:11,color:'#94a3b8',padding:'2px 0 2px 14px'}}><span>↳ Dr. {n}</span><span>- {fmt(Math.round(amt))}</span></div>))}</>
          ...
          {(()=>{
            ...
            return(<div style={{marginTop:8,paddingTop:8,borderTop:'1px solid #f5f5f5'}}>
              ...
              {due>0&&(payDocR===d.name?<CommPayForm docName={d.name} balance={due} onCancel={()=>setPayDocR(null)} onSave={async(amt,date,pay)=>{await settleRefPayment(db,actions,d.name,amt,date,pay,0);setPayDocR(null)}}/>
               :payDocR==='DED:'+d.name?<DeductCommForm db={db} docName={d.name} balance={due} onCancel={()=>setPayDocR(null)} onSave={async(clinAmt,labAmt,date)=>{await deductCommSplit(actions,d.name,date,clinAmt,labAmt);setPayDocR(null)}}/>
               :<div style={{display:'flex',gap:8}}>
