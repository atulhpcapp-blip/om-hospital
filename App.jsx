...
const SuperAdminDashboard=({onPreview=null})=>{
  ...
  const create=async()=>{
    ...
    if(he){
      if(he.message.includes('unique_phone')||he.message.includes('duplicate key')){
        // Find the existing hospital with this phone
        const {data:existing}=await supabase.from('hospitals').select('name').eq('phone',nH.phone.trim()).single()
        const hospName=existing?.name||'another hospital'
        setMsg({ok:false,t:'Phone '+nH.phone+' is already registered with: '+hospName})
        alert('❌ Phone already registered with: '+hospName)
      } else {
